...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, step_advantage_w=1.0, gigpo_mode="mean_std_norm", hgpo_type='hard', hgpo_history_length=2, hgpo_weight_type='mean', mode='',**kwargs):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        advantages, returns = core_hgpo.compute_hgpo_outcome_advantage(
            ...
            traj_index=data.non_tensor_batch['traj_uid'],
            types=hgpo_type,
            history_length=hgpo_history_length,
            weight_type=hgpo_weight_type,
            mode=mode
            )
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        batch = compute_advantage(
                            ...
                            gigpo_mode=self.config.algorithm.hgpo.mode,
                            hgpo_type=self.config.algorithm.hgpo.type,
                            hgpo_history_length=self.config.env.history_length,
                            hgpo_weight_type=self.config.algorithm.hgpo.weight_type,
